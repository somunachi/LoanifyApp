...
export const Extended = ({ selectAll, onAllCheckChange, searchQuery, setSearchQuery, filterStatus }) => {
  const [clientDataChecked, setClientDataChecked] = useState(false);
  const [checkboxesChecked, setCheckboxesChecked] = useState(Array(exdata.length).fill(false));

  const handleClientDataCheck = () => {
    const newClientDataChecked = !clientDataChecked;
    setClientDataChecked(newClientDataChecked);
    setCheckboxesChecked(Array(exdata.length).fill(newClientDataChecked));
  };
  ...
  const filteredData = filterStatus === 'All' ? exdata : exdata.filter((item) => item.status === filterStatus);
