...
export const DueLoans = ({ selectAll, onAllCheckChange, searchQuery, setSearchQuery, filterStatus }) => {
  const [clientDataChecked, setClientDataChecked] = useState(false);
  const [checkboxesChecked, setCheckboxesChecked] = useState(Array(duedata.length).fill(false));

  const handleClientDataCheck = () => {
    const newClientDataChecked = !clientDataChecked;
    setClientDataChecked(newClientDataChecked);
    setCheckboxesChecked(Array(duedata.length).fill(newClientDataChecked));
  };
  ...
  const filteredData = filterStatus === 'All' ? duedata : duedata.filter((item) => item.status === filterStatus);
      ...
      <div className={styles.clientContainer_block_block_block}>
        <div className={styles.clientContainer}>
          {filteredData.map((item, index) => {
            ...
            else if (item.status === 'Due for payment') {
              statusStyles.backgroundColor = '#F3B516';
              statusColor.textColor = '#fff';
